...
defineSupportCode(function ({ Given, When, Then, setDefaultTimeout }) {
    ...
    Given(/^Estou na página de Cadastro de contúdo ao sistema$/, async () => {
    ...
    Given(/^Preencho os campos de titulo "([^\"]*)" , descricao de "([^\"]*)" , introducao de "([^\"]*)" , desenvolvimento com titulo de "([^\"]*)" e descricao "([^\"]*)" e por fim, conclusao com "([^\"]*)"$/, async (titulo,descriConteudo, intro, nomeTopico, descriTopico ,concl) => {
    ...
    When(/^Eu tento adiconar o conteudo a lista de conteudos$/, async () => {
        await $("a[name='send']").click();
    })

    Then(/^Um alerta deve ser exibida devido com a mensagem "([^\"]*)"$/, async (alertMes) => {
